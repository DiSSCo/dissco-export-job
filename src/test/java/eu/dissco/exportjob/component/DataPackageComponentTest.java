...
class DataPackageComponentTest {
  ...
  void setUp() throws IOException {
    ...
    var template = configuration.getTemplate("data-package.ftl");
    this.dataPackageComponent = new DataPackageComponent(template, factory, JSON_MAPPER);
  }
  ...
  void testFormatDataPackage() throws IOException, FailedProcessingException {
    ...
    assertThat(datapackageNode.get("title").asString()).isEqualTo(
        "Naturalis Biodiversity Center (NL) - Tunicata");
    assertThat(datapackageNode.get("licenses").get(0).get("title").asString())
        .isEqualTo("CC-BY-4.0");
    assertThat(datapackageNode.get("resources").size()).isEqualTo(6);
    assertThat(datapackageNode.get("description").asString()).isEqualTo(
